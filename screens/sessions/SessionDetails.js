...
export default function SessionDetails() {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <TouchableOpacity
            style={{height: 40, width: 40, backgroundColor: "#00000080", position: 'absolute', marginTop: 2, top: insets.top, zIndex: 9, borderRadius: 40, left: 20, justifyContent: 'center', alignItems: 'center'}}
            onPress={() => navigation.navigate('Les séances')}
            >
                <Image source={backIcon} resizeMode='contain' style={{alignItems: 'center', justifyContent: 'center', height: 30}}/>
            </TouchableOpacity>
            ...
            <ScrollView style={styles.mainContainer}>
                ...
                <View style={styles.descriptionContainer}>
                    ...
                    <View style={{ marginBottom: 15 }}>
                        ...
                        <Text><Text style={{ fontWeight: '600' }}>Temps d'entraînement : </Text>{data.duration} minutes</Text>
                    ...
                    <View style={{ flexDirection: 'row', marginVertical: 10 }}>
                        {data.description.gain.map((gain, index) => {
                            return (
                                <View key={index} style={{height: 20, justifyContent: 'center', backgroundColor: "#18C07A", borderRadius: 10, marginRight: 7, paddingHorizontal: 10}}>
                        ...
                        {data.equipments.map((equipment, index) => {
                            return (
                                <View key={index} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5, flexWrap: 'wrap' }}>
                                    ...
                                    <Text>{equipment.name}</Text>{equipment.optional ? <Text style={{ color: 'grey', fontStyle: 'italic' }}> (facultatif)</Text> : null}
                                </View>
                            )
                        })}
                    </View>
            ...
            <View style={{marginBottom: insets.bottom, alignItems: 'center', paddingVertical: 20, backgroundColor:'#FFFFFF00'}}>
                <TouchableOpacity
                    onPress={() => navigation.navigate('Session', {name : 'SessionMain'})}
